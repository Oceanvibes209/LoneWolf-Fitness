...
function Home() {
  ...
  const handleDelete = async (id) => { // Assume `id` is passed to this function
    // Confirm deletion
    ...
    try {
      const response = await fetch(`/fitness_tracker/${id}`, { // Use template literals to embed the id
        method: "DELETE",
        headers: {
          "Content-Type": "application/json",
        },
  ...
  return (
    <div className="home-background">
          ...
          <ul>
            {workoutData.map((record) => (
              <li key={record.id}>
                ...
                {record.reps}, Weight: {record.weight} LBS, <button onClick={() => handleDelete(record.id)} className="dataDeleteBtn">x</button>
              </li>
            ))}
          </ul>
        ) : (
      ...
      <Link to={'/FoodTracker'}><button>Food Tracker</button></Link>
    </div>
  );
}
